document.addEventListener('DOMContentLoaded', function() {
    ...
    if (trainingForm) {
        trainingForm.addEventListener('submit', async function(event) {
            ...
            if (!fileInput.files[0]) {
                showTrainingMessage('Please select a file to upload.', 'error');
                return;
            }
            
            // Show loading state
            uploadTrainingBtn.disabled = true;
            uploadTrainingBtn.innerHTML = '<span>⏳</span> Uploading...';
            showTrainingMessage('Uploading and processing file...', 'loading');
            ...
            try {
                ...
                if (result.success) {
                    showTrainingMessage(`✅ ${result.message}`, 'success');
                    trainingForm.reset();
                    await loadTrainingData();
                } else {
                    showTrainingMessage(`❌ Error: ${result.error}`, 'error');
                }
            } catch (error) {
                console.error('Error uploading training data:', error);
                showTrainingMessage(`❌ Network error: ${error.message}`, 'error');
            } finally {
                uploadTrainingBtn.disabled = false;
                uploadTrainingBtn.innerHTML = '<span>📚</span> Upload & Train';
            }
        });
    }
    ...
    async function loadTrainingData() {
        if (!trainingDataList) return;
        
        try {
            trainingDataList.innerHTML = '<div class="loading">Loading training data...</div>';
            
            const response = await fetch('/api/train/data');
            const result = await response.json();
            
            if (result.success) {
                displayTrainingData(result.data);
            } else {
                trainingDataList.innerHTML = '<div class="error">Failed to load training data</div>';
            }
        } catch (error) {
    ...
    function displayTrainingData(data) {
        if (data.length === 0) {
            trainingDataList.innerHTML = `
                <div class="no-data">
                    No training data found. Upload your first file above.
                </div>
            `;
            return;
        }
        ...
        const fileTypes = [...new Set(data.map(item => item.fileType))];
        
        // Add stats cards
        const statsHtml = `
            <div class="training-stats">
                <div class="training-stat-card">
                    <div class="training-stat-number">${totalItems}</div>
                    <div class="training-stat-label">Items</div>
                </div>
                <div class="training-stat-card">
                    <div class="training-stat-number">${categories.length}</div>
                    <div class="training-stat-label">Categories</div>
                </div>
                <div class="training-stat-card">
                    <div class="training-stat-number">${sources.length}</div>
                    <div class="training-stat-label">Sources</div>
                </div>
                <div class="training-stat-card">
                    <div class="training-stat-number">${fileTypes.length}</div>
                    <div class="training-stat-label">File Types</div>
                </div>
            </div>
        `;
        
        const dataHtml = data.map(item => {
            // Create a file type indicator
            const fileTypeClass = `file-type-${item.fileType || 'txt'}`;
            const fileTypeIndicator = `<span class="file-type-indicator ${fileTypeClass}">${item.fileType || 'TXT'}</span>`;
            
            return `
                <div class="training-item" data-id="${item._id}">
                    <div class="training-header">
                        <h4>${escapeHtml(item.title)}${fileTypeIndicator}</h4>
                    ...
                    <div class="training-actions">
                        <button class="btn btn-small btn-danger" onclick="deleteTrainingData('${item._id}')">
                            <span>🗑️</span> Delete
                        </button>
                    </div>
                </div>
        ...
        trainingDataList.innerHTML = statsHtml + dataHtml;
    }
    ...
    window.deleteTrainingData = async function(id) {
        ...
        try {
            const response = await fetch(`/api/train/data/${id}`, {
                method: 'DELETE'
            });
            ...
            if (result.success) {
                showTrainingMessage('✅ Training data deleted successfully', 'success');
                
                // Remove item with animation
                const item = document.querySelector(`.training-item[data-id="${id}"]`);
                if (item) {
                    ...
                    setTimeout(() => {
                        ...
                        // Show "no data" message if all items are removed
                        if (trainingDataList.children.length === 0) {
                            trainingDataList.innerHTML = `
                                <div class="no-data">
                                    No training data found. Upload your first file above.
                                </div>
    ...
    function showTrainingMessage(message, type) {
        trainingStatusMessage.innerHTML = message;
        trainingStatusMessage.className = `status-message ${type}`;
        
        // Auto-hide success messages
        if (type === 'success') {
            setTimeout(() => {
                trainingStatusMessage.innerHTML = '';
                trainingStatusMessage.className = '';
            }, 5000);
        }
    }

    // Load initial data including training data
    loadDashboardData();
    loadTrainingData();
    ...
    window.toggleFormatGuide = function() {
        const formatGuide = document.getElementById('format-guide');
        if (formatGuide) {
            const isVisible = formatGuide.style.display !== 'none';
            formatGuide.style.display = isVisible ? 'none' : 'block';
            
            // Scroll to the guide if now visible
            if (!isVisible) {
                formatGuide.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
            }
        }
    };
});